        ...
        public bool Edit(RequestVisitRecordDTO request, string operUserID, out string reason)
        {
            ...
            using (var db = new DBEntities())
            {
                ...
                if (model != null)
                {
                    ...
                    model.VisitTag = request.VisitTag;
                    ...
                    model.MianDepartment = request.MianDepartment;
        ...
        public ResponseVisitRecordDTO GetDetail(RequestVisitRecordQDTO request)
        {
            ...
            using (var db = new DBEntities())
            {
                ...
                if (model != null)
                {
                    ...
                    ret.MianPersons = personq.Where(t => t.OwenType == EnumPersonOwenType.MainHanle).ToList();
                    ret.OurPersons = personq.Where(t => t.OwenType == EnumPersonOwenType.Our && t.Level == 0).ToList();
                    ret.OurOtherPersons = personq.Where(t => t.OwenType == EnumPersonOwenType.Our && t.Level == 1).ToList();
                    ret.TheyMainPersons = personq.Where(t => t.OwenType == EnumPersonOwenType.They && t.Level == 0).ToList();
                    ret.TheyOtherPersons = personq.Where(t => t.OwenType == EnumPersonOwenType.They && t.Level == 1).ToList();
                    ...
                    ret.OurOrgs = orgq.Where(t => t.OwenType == EnumOrgOwenType.Our).ToList();
                    ret.TheyOrgs = orgq.Where(t => t.OwenType == EnumOrgOwenType.They).ToList();
                    ret.BeViOrgs = orgq.Where(t => t.OwenType == EnumOrgOwenType.BeVi).ToList();

                    var fileq = from vifile in db.VisitFiles
                                select vifile;

                    ret.SJWLFiles = fileq.Where(t => t.Type == "11").ToList();
                    ret.LBWLFiles = fileq.Where(t => t.Type == "12").ToList();
                    ret.NBGLFiles = fileq.Where(t => t.Type == "13").ToList();
                    ret.HYXGFiles = fileq.Where(t => t.Type == "14").ToList();
                    ret.NewsFiles = fileq.Where(t => t.Type == "15").ToList();
                    ret.OtherFiles = fileq.Where(t => t.Type == "16").ToList();
            ...
            using (var db = new DBEntities())
            {
                var query = from record in db.VisitRecords
                            ...
                            select new ResponseVisitListDTO()
                            {
                                ...
                                MianDepartment = record.MianDepartment,
                ...
                if (!string.IsNullOrEmpty(request.MianDepartment))
                {
                    query = query.Where(t => t.MianDepartment.Contains(request.MianDepartment));
                }
